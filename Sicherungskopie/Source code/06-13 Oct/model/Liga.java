...
public class Liga {
	...
	// TODO delete, you can't add a team. Size of league can't be changed except for an unprobable event. Do as in group!
	public void addNewTeam(String str, boolean fromOptions) throws Exception {
		// Speicherung der Daten in String-Arrays
		String[] namen = new String[this.numberOfTeams + 1];
		String[] gruendungsdaten = new String[this.numberOfTeams + 1];
		...
		if (numberOfTeams >= 2)		numberOfMatchdays = 4 * halbeanzMSAuf - 2;
	...
	public void laden(int index) {
		String saison;
		if (this.isSummerToSpringSeason)	saison = saisons[index] + "_" + (saisons[index] + 1);
		else								saison = "" + saisons[index];
...
//        testIsErgebnisplanEntered();
	...
	public int getDate(int matchday) {
		return datesAndTimes[matchday][0];
	}
...
//    	this.jListConfigModel.clear();
	...
	private void spielplanLaden() {
		try {
	        ...
	        for (counter = 0; counter < this.numberOfMatchdays; counter++) {
	        	String[] inhalte = this.spielplanFromFile[counter + 1].split(";");
	            ...
	            if (!this.isSpielplanFullyEmpty(counter)) {
	            	...
	            	setDate(counter, Integer.parseInt(uhrzeiten[0]));
	            	for (match = 0; (match + 1) < uhrzeiten.length; match++) {
	            		setKOTIndex(counter, match, Integer.parseInt(uhrzeiten[match + 1]));
            		}
	...
	private void fromString(String daten) {
		...
		this.numberOfTeams = Integer.parseInt(daten.substring(daten.indexOf("A_MS*") + 5, daten.indexOf(";A_CL*")));
