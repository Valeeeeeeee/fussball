...
public class MyDateChooser extends JFrame {
	...
	private String[] wt_kurz = {"Mo", "Di", "Mi", "Do", "Fr", "Sa", "So"};	// TODO Combobox mit Standard m�glichkeiten (lg.tageseitfr && anst.zeitn) 
																			//und eine M�glichkeit zum eingeben eines speziellen datums (neue ausnahmeanstosszeit)
	
	public MyDateChooser(Liga lg) {
		super();
		initGUI(lg);
	}
	...
	public void initGUI(Liga lg) {
		...
		startjahr = lg.saisons[lg.aktuelle_saison];
		def_starttag = lg.default_starttag;
		...
		}
		{
	        ...
	        jCBStDay.addItemListener(new ItemListener() {
	            public void itemStateChanged(ItemEvent evt) {
	                jCBStDayItemStateChanged(evt, lgfinal);
	            }
	        });
		}
		{
	        ...
	        jCBStMonth.addItemListener(new ItemListener() {
	            public void itemStateChanged(ItemEvent evt) {
	            	jCBMonthItemStateChanged(evt, lgfinal, true);
	            }
	        });
		}
		{
	        ...
	        jCBStYear.addItemListener(new ItemListener() {
	            public void itemStateChanged(ItemEvent evt) {
	                jCBYearItemStateChanged(evt, lgfinal, true);
	            }
	        });
		}
		{
		...
		}
		{
			...
			go.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
			        if (setDateFromCBs(lgfinal))		Start.Spieltage.datumeingegeben(date, aszindex, lgfinal);
				}
			});
		}
		{
...
//	        });
		...
		}
		{
	        ...
	        jCBMonth.addItemListener(new ItemListener() {
	            public void itemStateChanged(ItemEvent evt) {
	            	jCBMonthItemStateChanged(evt, lgfinal, false);
	            }
	        });
		...
		}
		{
	        ...
	        jCBYear.addItemListener(new ItemListener() {
	            public void itemStateChanged(ItemEvent evt) {
	                jCBYearItemStateChanged(evt, lgfinal, false);
	            }
	        });
	...
	public void setDate(int date, int aszindex) {
		try {
			...
			JOptionPane.showMessageDialog(null, "Es wurde ein illegales Argument �bergeben!");
	...
	public boolean setDateFromCBs(Liga lg) {
		...
		if ((greg.get(Calendar.DAY_OF_WEEK) - 2) != def_starttag) {
			int cancel = JOptionPane.showConfirmDialog(null,	"Der angegebene Tag entspricht nicht dem standardm��igen Starttag f�r einen Spieltag. (" + wochentage[def_starttag] + ") \n" +
																"M�chtest du trotzdem fortfahren?", "Bist du sicher?", JOptionPane.YES_NO_OPTION);
		...
		if (this.jCBAnstosszeiten.getSelectedIndex() == (jCBAnstosszeiten.getModel().getSize() - 1)) { // dann wurde eine neue Anstosszeit ausgew�hlt
			...
			for (int i = 0; i < lg.tageseitfr.length; i++) {
				if (lg.tageseitfr[i] == diff) {
					if(lg.anstosszeiten[i].equals(timenewasz)) {
						this.aszindex = i;
						break;
					}
				}
			...
			if (this.aszindex == (jCBAnstosszeiten.getModel().getSize() - 1)) {
				int[] oldtageseitfr = lg.tageseitfr;
				String[] oldanstosszeiten = lg.anstosszeiten;
				lg.tageseitfr = new int[oldtageseitfr.length + 1];
				lg.anstosszeiten = new String[oldanstosszeiten.length + 1];
				
				for (int i = 0; i < oldtageseitfr.length; i++) {
					lg.tageseitfr[i] = oldtageseitfr[i];
					lg.anstosszeiten[i] = oldanstosszeiten[i];
				}
				
				lg.tageseitfr[lg.tageseitfr.length - 1] = diff;
				lg.anstosszeiten[lg.anstosszeiten.length - 1] = timenewasz;
			} else {
	...
	public void jCBYearItemStateChanged(ItemEvent evt, Liga lg, boolean starttag) {
	...
	public void jCBMonthItemStateChanged(ItemEvent evt, Liga lg, boolean starttag) {
		if (evt.getStateChange() == ItemEvent.SELECTED) {
			int monat, tag;
			if (starttag) {
				monat = jCBStMonth.getSelectedIndex() + 1;
	...
	public void jCBStDayItemStateChanged(ItemEvent evt, Liga lg) {
		if (evt.getStateChange() == ItemEvent.SELECTED) {
			cbasz_aktualisieren(lg);
		}
	}
	
	public void cbasz_aktualisieren(Liga lg) {
		GregorianCalendar greg = new GregorianCalendar(jCBStYear.getSelectedIndex() + startjahr, jCBStMonth.getSelectedIndex(), jCBStDay.getSelectedIndex() + 1);
		...
		String[] asz = new String[lg.anstosszeiten.length + 1];
		for (int i = 0; i < (asz.length - 1); i++) {
			asz[i] = wt_kurz[(def_starttag + dayofweek + 1 + lg.tageseitfr[i]) % 7] + " " + lg.anstosszeiten[i];
		}
		asz[asz.length - 1] = "anderes";
		ComboBoxModel jCBAnstosszeitenModel = new DefaultComboBoxModel(asz);
