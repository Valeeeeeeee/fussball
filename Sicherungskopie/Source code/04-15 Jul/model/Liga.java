...
//			ergebnis[spieltag.current_matchday][i % anzahl_spiele][i / anzahl_spiele] = Integer.parseInt(spieltag.tore[i].getText());
			setErgebnis(spieltag.current_matchday, i % anzahl_spiele, i / anzahl_spiele, Integer.parseInt(spieltag.tore[i].getText()));
		}
		int counter = 0;
		for (int i = 0; i < mannschaften.length; i++) {
			...
			if (mannschaften[i].getMatch(spieltag.current_matchday)[1] != 0) { 
				...
				int gegentore = Integer.parseInt(spieltag.tore[(pos + anzahl_spiele) % spieltag.tore.length].getText());
	...
	public int getErgebnis(int matchday, int match, int backOrFront) {
		return ergebnis[matchday][match][backOrFront];
	}
	
	public void setErgebnis(int matchday, int match, int backOrFront, int goals) {
		ergebnis[matchday][match][backOrFront] = goals;
	}
	
	public void setErgebnisNew(int matchday, int match, int goalsLeft, int goalsRight) {
		ergebnis[matchday][match][0] = goalsLeft;
		ergebnis[matchday][match][1] = goalsRight;
	}
...
//		this.jListSpielplanModel.clear();
	...
	private void ergebnisse_laden() {
		try {
	        ...
	        for (counter = 0; counter < this.anzahl_spieltage; counter++) {
	            String e = this.ergebnisseFromFile[counter];
	            this.ergebnisseEingetragen[counter] = Boolean.parseBoolean(e.substring(0, e.indexOf(";")));
	            e = e.substring(e.indexOf(";") + 1);
	            int count2 = 0;
...
//	                    this.ergebnis[counter][count2][1] = toreAWAY;
		            	...
		            	setErgebnis(counter, count2, 0, -1);
		            	setErgebnis(counter, count2, 1, -1);
...
//	            	this.ergebnis[counter][k][1] = -1;
	            	...
	            	setErgebnis(counter, k, 0, -1);
	            	setErgebnis(counter, k, 1, -1);
	            }
	        }
...
//                    string += this.ergebnis[i][j][0] + ":" + this.ergebnis[i][j][1] + ";";
                    
                    string += getErgebnis(i, j, 0) + ":" + getErgebnis(i, j, 1) + ";";
                }
            }
