...
//				mannschaften[i].setBackground(null);
	...
	public void spieltagAnzeigen() {
		if (belongsToKORound) {
			// TODO spieltagAnzeigen
			return;
		}
		
//		int numberOfMatches; // bereits im Konstruktor
//		if (belongsToALeague)	numberOfMatches = liga.getAnzahlSpiele();
//		else					numberOfMatches = gruppe.getNumberOfMatchesPerMatchday();
		
		if (current_matchday == -1) {
			if (belongsToALeague)	current_matchday = liga.getCurrentMatchday();
			else					current_matchday = gruppe.getCurrentMatchday();
			...
			for (int i = 0; i < this.tore.length; i++) {
				if (belongsToALeague)	this.tore[i].setText("" + liga.ergebnis[current_matchday][i % numberOfMatches][i / numberOfMatches]);
				else					this.tore[i].setText("" + gruppe.ergebnisse[current_matchday][i % numberOfMatches][i / numberOfMatches]);
			}
			...
			// TODO Überprüfung ob Spieltag bereits eingetragen
			for (int i = 0; i < this.mannschaften.length; i++) {
				if (belongsToALeague)	this.mannschaften[i].setText(liga.getMannschaften()[liga.spieltage[current_matchday][i % numberOfMatches][i / numberOfMatches] - 1].getName());
				else					this.mannschaften[i].setText(gruppe.getMannschaften()[gruppe.spiele[current_matchday][i % numberOfMatches][i / numberOfMatches] - 1].getName());
				if (belongsToALeague)	this.tore[i].setText("" + liga.ergebnis[current_matchday][i % numberOfMatches][i / numberOfMatches]);
				else					this.tore[i].setText("" + gruppe.ergebnisse[current_matchday][i % numberOfMatches][i / numberOfMatches]);
			}
			...
			if (belongsToALeague) {
				if (!liga.spieltageEingetragen[current_matchday])	this.setTFsEditable(false);
			}
			else {
				if (!gruppe.spieleEingetragen[current_matchday])	this.setTFsEditable(false);
			}
	...
	public void mannschaftenButtonClicked(int index) {
		...
		else					mannschaften[edited_team].setText(gruppe.getMannschaften()[index].getName());
